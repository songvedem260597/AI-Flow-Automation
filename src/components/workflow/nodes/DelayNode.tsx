...
export const DelayNode: React.FC<NodeProps> = memo(({ data, selected }) => {
  const delayData = data as unknown as { duration?: number; label?: string }
  const seconds = Math.round((delayData.duration || 1000) / 1000)
    ...
    >
      <div className="flex items-center gap-2.5 px-3.5 py-3">
        ...
        <div className="flex-1">
          <span className="text-xs font-medium text-white/80">{delayData.label || 'Delay'}</span>
        </div>
        <span className="text-lg font-bold text-amber-400 font-mono">{seconds}</span>
