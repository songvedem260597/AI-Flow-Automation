      ...
      <div className="px-3 py-2.5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1.5 line-clamp-2 text-[8px] leading-3.5 text-white/42">{shot.description}</p>
          </div>
          <span className="shrink-0 rounded-full bg-[#7C5CFF]/10 px-2 py-1 text-[7px] text-[#B8A8FF]/70">{shot.status}</span>
        </div>
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          {shot.workflowNodeIds.length > 0 && onViewNode && <button type="button" onClick={() => onViewNode(shot.workflowNodeIds[0])} className="flex h-6 items-center gap-1.5 rounded-lg px-1.5 text-[8px] font-medium text-white/30 hover:bg-white/[0.04] hover:text-[#C8BCFF]"><LocateFixed className="h-3 w-3" /> View on canvas</button>}
          {reviewApproval && onApprove && onRegenerate && onReject && (
            <>
              <button type="button" disabled={busy} onClick={() => onApprove(reviewApproval.id)} className="flex h-6 items-center gap-1 rounded-lg bg-[#7C5CFF]/16 px-2 text-[8px] font-semibold text-[#C8BCFF] hover:bg-[#7C5CFF]/24 disabled:opacity-35"><Check className="h-3 w-3" /> Approve {reviewKind}</button>
              <button type="button" disabled={busy} onClick={() => onRegenerate(reviewApproval.id, prompt)} className="flex h-6 items-center gap-1 rounded-lg px-2 text-[8px] text-white/34 hover:bg-white/[0.05] hover:text-white/68 disabled:opacity-35"><RotateCcw className="h-3 w-3" /> Regenerate</button>
          ...
          {!reviewApproval && retryKind && onPrepareRetry && <button type="button" disabled={busy} onClick={() => onPrepareRetry(shot.id, retryKind)} className="flex h-6 items-center gap-1 rounded-lg bg-amber-300/[0.07] px-2 text-[8px] font-medium text-amber-100/55 hover:bg-amber-300/[0.11] hover:text-amber-100/75 disabled:opacity-35"><RotateCcw className="h-3 w-3" /> Prepare {retryKind} retry</button>}
        </div>
      </div>
    </div>
  )
...
export const AgentScenes: React.FC<{
  ...
  return <div className="space-y-3 p-4">{[...project.scenes].sort((a, b) => a.order - b.order).map((scene) => <section key={scene.id} className="overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.018]"><div className="border-b border-white/[0.06] px-3.5 py-3"><div className="flex items-center justify-between"><h3 className="text-[10px] font-semibold text-white/72">{String(scene.order).padStart(2, '0')} · {scene.title}</h3><span className="text-[8px] text-white/24">{scene.shotIds.length} shots</span></div><p className="mt-1.5 text-[8px] leading-3.5 text-white/32">{scene.summary}</p></div><div className="space-y-1.5 p-2">{scene.shotIds.map((shotId) => shotsById.get(shotId)).filter(Boolean).map((shot) => shot && <ShotCard key={shot.id} shot={shot} reviewApproval={pendingReviewByShot.get(shot.id)} busy={busyApprovalId === pendingReviewByShot.get(shot.id)?.id} onViewNode={onViewNode} onApprove={onApprove} onRegenerate={onRegenerate} onReject={onReject} retryKind={retryKindForShot(shot.id)} onPrepareRetry={onPrepareRetry} />)}</div></section>)}</div>
}
